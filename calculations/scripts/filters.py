...
def remove_wiggles(x, y, p):
    ...
    fwiggle = interpolate.UnivariateSpline(x, y, k=3, s=0)
    ...
    d2 = interpolate.UnivariateSpline(x, derivs[2], k=3, s=1.0)

    wzeros = d2.roots()
    wtrend = interpolate.UnivariateSpline(wzeros, fwiggle(wzeros), k=3, s=0)
...
def moving_average(x, signal, w):
    ...
    X = np.linspace(x[0], x[-1], len(signal_filtered))

    signal_interpolated = interpolate.UnivariateSpline(X, signal_filtered, k=3, s=0)
