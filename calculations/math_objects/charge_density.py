...
def normalized_eigenstate(self, eigenstate, integration_limit=300):
    """
    ...
    rho_without_normalization = (
        lambda z: (eigenvalue - self.e * self.A0_field(z))
        * np.abs(eigenstate.sol(z)[0]) ** 2
    )

    # calculating its norm
    rho_norm_squared = mpmath.quad(rho_without_normalization, [0, 1])

    rho_normalized = lambda z: np.sign(eigenvalue) * (
        rho_without_normalization(z) / rho_norm_squared
    )
    ...
    def total_rho(z, renormalization=renormalization):
        ...
        if renormalization:
            renormalization_closed_form = (
                self.e * self.lambda_value * z * (1 - z) / np.tan(np.pi * z)
            )
        else:
