...
def __init__(
    ...
    self.A0_field = Vector_Potential(n_points=self.n_points)
    self.bcs = bcs
    ...
    if eigenstate_array is None:
        try:
            ...
            if eigenvalue_array is not None:
    ...
    if not A0_perturbation is None:
        ...
        self.A0_field.value = self.A0_base_value + self.A0_perturbation
    else:
        self.A0_value = self.A0_base_value
    #print('from init', self.A0_field.value + self.lambda_value*(self.z-1/2))
