...
def value_generator(starting_value, ratio, method="linear"):
    ...
    """
    methods = {
        "linear": lambda x, ratio: ratio * x,
        "quadratic": lambda x, ratio: ratio * x**2,
        "logarithmic": lambda x, ratio: ratio
        * np.log(1 + x),  # So that it starts in 0, not 1
    ...
    if method not in methods:
        if isinstance(method, str):
            raise ValueError(
                f"The specified method {method} is not in {methods.keys()}."
            )
            ...
            try:
                ...
                raise TypeError(
                    "The supplied function is not of the form f(starting_value, ratio)"
                )
    else:
        f = methods[method]
