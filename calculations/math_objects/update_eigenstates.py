...
def update_eigenstates(
    ...
    self.A0_perturbation = (
        A0_perturbation.sol(self.z)[0] - A0_perturbation.sol(0.5)[0]
    )  # Back to original shape and shifted to go to 0 at z=1/2
...
#        plt.plot(self.A0_field.value+ self.lambda_value*(self.z-1/2), fmt, alpha=alpha, label = f'iteration nº {i}')
        ...
        if plot:
            if axis is not None:
                ...
                axis.plot(self.z, self.rho_array, "b", alpha=alpha)
                axis.plot(self.z, self.A0_perturbation, "r", alpha=alpha)
#    plt.legend(loc='best')
#    plt.show()
