...
def calculate_eigenstates(self,
    ...
    Parameters:
        tolerance: float=1e-2. The tolerance for which the KG are submitted to
    ...
    for eigenstate_guess, eigenstate_gradient_guess, eigenvalue_guess in zip(
        ...
        if float_in_array(true_eigenvalue, true_eigenvalues_array, tol=self.float_tol) or not true_eigenstate.success:
