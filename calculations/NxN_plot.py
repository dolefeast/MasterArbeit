...
for lambda_index, lambda_value in enumerate(range(lower, higher, (higher-lower)//n_images)):
    for scalar_index, scalar_mass in enumerate(range(lower, higher, (higher-lower)//n_images)):
        ...
        for omega_n in range(1, 100, 30):
            ...
            if float_in_array(solution.p[0], omegas):
                ...
                ax[lambda_index, scalar_index].plot(x, (solution.p[0]- systesm.phi.charge*systesm.A0(x))*np.real(solution.sol(x)[0])**2/scalar_mass/lambda_value, fmt)
                ax[lambda_index, scalar_index].set_title(f'lambda={lambda_value}, mass={scalar_mass}')
...
phi_n_dirichlet = lambda z, n: (phi.mass**2 + np.pi**2 * n**2) ** (-1/4) * (np.sin(np.pi*n*z) + lambda_value * np.sqrt(phi.mass**2 + np.pi**2 * n**2) / (2*np.pi*np.abs(n)) * ((1/2 - z)/np.pi/n * np.sin(np.pi*n*z) - z*(1-z) * np.cos(np.pi*n*z))  )
