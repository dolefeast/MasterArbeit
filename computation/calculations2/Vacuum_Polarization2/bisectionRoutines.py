def findRootBisection(self, fun, x1, x2, tol=1e-15, maxiter=800):
    ...
    if fun(x1)*fun(c) < 0: # Means the root was at the left of c
        return self.findRootBisection(fun, x1, c, tol=self.bisectionTol, maxiter=maxiter-1)
    else:
        return self.findRootBisection(fun, c, x2, tol=self.bisectionTol, maxiter=maxiter-1)
