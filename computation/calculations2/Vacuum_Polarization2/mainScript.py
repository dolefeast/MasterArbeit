...
def antisymmetrize(array):
    return np.array( [ (x - y) for x, y in zip(array, array[::-1])])
...
def constantLambdaIterations(self):
    ...
    while self.n < self.maxNIterations:
        ...
        if self.savePlots:
            self.fig4.savefig(
                    "figures/"+self.directory+"/"+self.bcs+"/A0InducedintermediateStepsLambdaValue_"+self.floatToStr(self.lambdaValue)+".pdf"
                    )
            self.fig5.savefig(
                    "figures/"+self.directory+"/"+self.bcs+"/omegaintermediateStepsLambdaValue_"+self.floatToStr(self.lambdaValue)+".pdf"
                    )
...
def walkforward(self):
    ...
    self.lambdaStep /= self.walkbackFactor*4
    self.lambdaValue += self.lambdaStep
    if self.a!=0:
        newRelaxParameter = self.relaxParameter / self.walkbackFactor*4
        if newRelaxParameter < 1:
            self.relaxParameter = newRelaxParameter
...
def fullScript(self):
    ...
    while self.lambdaValue < self.lambdaMax:
        ...
        try:
            ...
            successCount += 1 # The calculation was successful
            if successCount == 5:
                self.walkforward()
                successCount = 0
