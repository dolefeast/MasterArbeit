...
def read_solutions_from_file(self):
    ...
    self.rho_array  = np.genfromtxt(
        f"saved_solutions/{self.bcs}/rho/{file_id}",
        dtype=float,
        delimiter="\n",
    )

    self.A0_induced = np.genfromtxt(
        f"saved_solutions/{self.bcs}/A0_induced/{file_id}",
        dtype=float,
        delimiter=",",
    )
    ...
    self.eigenstate_array =  list(
        np.genfromtxt(
        f"saved_solutions/{self.bcs}/eigenstate_array/{file_id}", delimiter=","
        )
    )
    ...
    self.eigenvalue_array = list(
        np.genfromtxt(
        f"saved_solutions/{self.bcs}/eigenvalue_array/{file_id}", delimiter="\n"
        )
    )
